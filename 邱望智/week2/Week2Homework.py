...
def evaluate(model):
    ...
    sum_y = np.sum(np.array(y), axis=0)
    for i in range(len(sum_y)):
        print("本次预测集中共有%d个【%d】类样本" % (sum_y[i], i + 1))
    correct, wrong = 0, 0
    with torch.no_grad():
        y_pred = model(x)  # 模型预测 model.forward(x)
        for y_p, y_t in zip(y_pred, y):  # 与真实标签进行对比
            y_p_max_index = np.argmax(y_p)
            y_t_max_index = np.argmax(y_t)
            if y_p_max_index == y_t_max_index:
                correct += 1  # 预测分类与实际分类一致，判断正确
            else:
                wrong += 1  # 反之预测分类与实际分类不一致，判断错误
...
def predict(model_path, input_vec):
    ...
    for vec, res in zip(input_vec, result):
        print("输入：%s, 预测类别：%d, 概率值：%s" % (vec, np.argmax(res) + 1, res))  # 打印结果
