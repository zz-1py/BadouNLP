...
def build_sample():
    ...
    # 随机五维张量中 最大值对应的索引的值为1 其余为0
    if a == 0:
        return x, [1, 0, 0, 0, 0] # A类
    if a == 1:
        return x, [0, 1, 0, 0, 0] # B类
    if a == 2:
        return x, [0, 0, 1, 0, 0] # C类
    if a == 3:
        return x, [0, 0, 0, 1, 0] # D类
    if a == 4:
        return x, [0, 0, 0, 0, 1] # E类
...
def build_dataset(total_sample_num): #total_sample_num 为想要获得的训练样本的数量
    ...
    for i in range(total_sample_num):
        x, y = build_sample()
        X.append(x)
        Y.append(y) #训练样本中的输出已经为张量 所以不用加[]
...
def evaluate(model):
    ...
    with torch.no_grad():
        ...
        for y_p, y_t in zip(y_pred, y):  # 与真实标签进行对比
            # 真实值和预测值中的最大值的索引位置相同即认为预测正确  否则错误
...
def main():
    ...
    # 保存模型
    torch.save(model.state_dict(), "model.bin1")
    # 画图
    # print(log)
