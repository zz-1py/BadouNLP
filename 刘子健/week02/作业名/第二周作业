...
基于pytorch框架编写模型训练
实现一个五分类任务：五维随机向量最大的数字在哪维就属于哪一类
...
class TorchModel(nn.Module):
    def __init__(self, input_size, num_classes):
        ...
        self.linear = nn.Linear(input_size, num_classes)  # 线性层，输出为五类
    ...
    def forward(self, x, y=None):
        ...
        if y is not None:
            ...
            return x  # 输出原始预测结果（logits）

# 生成一个样本, 样本的生成方法，代表了我们要学习的规律
# 随机生成一个5维向量，其最大值的索引即为类别标签
def build_sample():
    x = np.random.random(5)
    y = np.argmax(x)  # 获取最大值的索引作为类别标签
    return x, y
