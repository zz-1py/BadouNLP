...
规律：x是一个5维向量，分为五类，每一类为的向量中最大值的下标
...
def evaluate(model):
    ...
    with torch.no_grad():
        ...
        for y_p, y_t in zip(y_pred, y):  # 与真实标签进行对比
            max_index_y_p = torch.argmax(y_p).item()
            # 获取 y_t 中最大值的索引
            max_index_y_t = torch.argmax(y_t).item()
            if max_index_y_p == max_index_y_t:
                correct += 1  # 判断正确
            else:
                wrong += 1  # 判断错误
