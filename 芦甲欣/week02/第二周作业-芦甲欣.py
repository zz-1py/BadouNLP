...
# 随机生成一个5维向量，x是一个5维向量，哪一维数据大，则为几类数据
def build_sample():
...
def predict(model_path, input_vecs):
    input_size = 5
    output_size = 5  # 正确的输出大小
    model = TorchModel(input_size, output_size)
    model.load_state_dict(torch.load(model_path))  # 加载训练好的权重
    model.eval()  # 测试模式
    with torch.no_grad():  # 不计算梯度
        results = model.forward(torch.FloatTensor(input_vecs))  # 模型预测
        for vec, res in zip(input_vecs, results):
            pred_index = torch.argmax(res, dim=0).item()  # 获取预测的类别索引
            print("输入：%s, 预测类别：%d ,第 %d 个数字为最大值" % (vec, pred_index, pred_index+1))  # 打印结果


if __name__ == "__main__":
    main()
    test_vec = [[0.97889086, 0.15229675, 0.31082123, 0.03504317, 0.88920843],
                [0.99963533, 0.5524256, 0.95758807, 0.95520434, 0.84890681],
