...
def predict(model_path, input_vec):
    ...
    for vec, res in zip(input_vec, result):
        predicted_class = torch.argmax(res, dim=0).item()  # 获取最大概率的索引
        print("输入：%s, 预测类别：%d, 概率值：%f" % (vec, predicted_class+1, res[predicted_class].item()))  # 打印结果
