...
def evaluate(model):
    ...
    print("本次预测集中共有%d个1类别，%d个2类别，%d个3类别，%d个4类别，%d个5类别"
          % (sum(c == 0 for c in y),sum(c == 1 for c in y),sum(c == 2 for c in y),sum(c == 3 for c in y),sum(c == 4 for c in y)))
    correct, wrong = 0, 0
    with torch.no_grad():
        y_pred = model(x)  # 模型预测
...
def predict(model_path, input_vec):
    ...
    for vec, res in zip(input_vec, result):
        ...
        print("输入：%s, 预测类别：第%d类别" % (vec, max_index + 1))  # 打印结果
