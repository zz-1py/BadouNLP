...
class TorchModel(nn.Module):
    def __init__(self, input_size):
        ...
        self.linear1 = nn.Linear(input_size, input_size*2)  # 线性层1
        self.linear2 = nn.Linear(input_size*2, input_size*2)  # 线性层2
        self.linear3 = nn.Linear(input_size * 2, input_size)  # 线性层2
        # 保证神经网络输出和正确数据一致性
...
def evaluate(model):
    ...
    with torch.no_grad():
        ...
        for y_p, y_t in zip(y_pred, y):  # 与真实标签进行对比
            ...
            t_var, t_index = torch.max(y_t, dim=0)
            if t_index == p_index:
                correct += 1  # 判断正确
            else:
                wrong += 1
